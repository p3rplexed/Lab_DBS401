...
class order
{
    public function add()
    {
        ...
        $insert_cart = insert($sql_insert_cart);
        if (!$insert_cart) {
            return false;
        }
        ...
        $get_cart_last_id = select($sql_get_cart_last_id);
        if ($get_cart_last_id) {
        ...
        foreach ($cart_user as $key => $value) {
            ...
            $insert_order_details = insert($sql_insert_order_details);
        ...
        $delete_cart = delete($sql_delete_cart);
        if ($delete_cart) {
            return true;
        }
